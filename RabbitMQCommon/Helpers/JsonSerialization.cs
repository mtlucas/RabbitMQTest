...
namespace RabbitMQCommon
{
    ...
    public static class JsonSerialization
    {
        ...
        /// <summary>
        /// Reads all objects from an Json file separated by linefeed (these were appended).
        /// <para>Object type must List.</para>
        /// </summary>
        /// <param name="filePath">The file path to read the object instance from.</param>
        /// <returns>Returns a new List of the objects read from the Json file.</returns>
        public static List<object> ReadMultipleLinesFromJsonFile(string filePath)
        {
            TextReader reader = null;
            ...
            try
            {
                using (reader = new StreamReader(filePath))
                {
                    ...
                    for (int i = 0; i < content.Length; i++)
                    {
                        ...
                        {
                            jsonArray.Add(Newtonsoft.Json.JsonConvert.DeserializeObject(content[i]));
                        }
                    }
                }
                return jsonArray;
